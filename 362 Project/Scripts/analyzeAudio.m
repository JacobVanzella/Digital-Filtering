...
function DFT = analyzeAudio(myRecording, Fs, N)
    ...
    %% 2.3: Plot dB scale amplitude of DFT.
    subplot(3,1,2)
    plot(frequency, 20*log10(DFT_AbsShift))
    xlabel('Frequency (Hz)')
    ylabel('Frequency Amplitude (dB)')
    
    %% 2.4: Downsamples DFT to simplify phase angle plot.
    downsampledFs = Fs/4;
    downsampledN = 80000 / downsampledFs;
    DFT_Shift = downsample(DFT_Shift, downsampledN);
    frequency = downsample(frequency, downsampledN);
    
    %% 2.5: Plot downsampled DFT phase angle.
    subplot(3,1,3)
